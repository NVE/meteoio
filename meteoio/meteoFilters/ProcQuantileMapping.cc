...
ProcQuantileMapping::ProcQuantileMapping(const std::vector< std::pair<std::string, std::string> >& vecArgs, const std::string& name, const std::string& i_root_path)
        : ProcessingBlock(vecArgs, name), quantiles(), corrections(), root_path(i_root_path), period('a')
{
	parse_args(vecArgs);
                        ...
                        std::vector<MeteoData>& ovec)
{
	...
	if (period=='a') 
		correctPeriod(param, 0, ovec.size() - 1, ovec);
	else 
		throw InvalidArgumentException("Periods other than 'ALL' are not supported yet by Filters::"+block_name, AT);
}

void ProcQuantileMapping::correctPeriod(const unsigned int& param, const size_t& idx_start, const size_t& idx_end, std::vector<MeteoData>& ovec) const
{
	std::vector<double> vecX(idx_end - idx_start + 1);
	for (size_t ii=idx_start; ii<=idx_end; ii++){
		vecX[ii] = ovec[ii]( param );
	}

	const std::vector<double> thresholds( Interpol1D::quantiles(vecX, quantiles) ); //compute the thresholds for each bin
	...
	for (size_t ii=idx_start; ii<=idx_end; ii++){
		...
		tmp *= getCorrection(thresholds, tmp);
	}
}
