...
void GeotopIO::writeMeteoData( const std::vector<std::vector<MeteoData> >& vecMeteo,
	...
	if (!IOUtils::validFileAndPath(meteoFile)) throw InvalidNameException(meteoFile,AT);
	...
	for (size_t ii = 0; ii < vecMeteo.size(); ii++) {
		...
		if (!IOUtils::validFileAndPath(ss.str())) throw InvalidNameException(ss.str(),AT);
...
void GeotopIO::readMetaData(const std::string& metafile) {
	...
	if (!IOUtils::validFileAndPath(metafile)) throw InvalidNameException(metafile, AT);
	if (!IOUtils::fileExists(metafile)) throw NotFoundException(metafile, AT);
