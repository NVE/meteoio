...
namespace mio {
     ...
     *
     * @brief The Non-Binary Environmental Data Archive (NEAD) format is a versatile and intuitive format that merges the self-documenting
     ...
     * <a href="https://github.com/GEUS-Glaciology-and-Climate/NEAD">official format documentation</a>.
     *
     * When using NEAD with MeteoIO, the following additional MetaData is required:
     ...
     * directory
     * will be scanned for files ending in ".nead" and sorted in ascending order;
     ...
     * - NEAD_APPEND: when an output file already exists, should the plugin try to append data (default: false); [Output] section
     * - NEAD_OVERWRITE: when an output file already exists, should the plugin overwrite it (default: true)? [Output] section
     ...
     * - NEAD_SEPARATOR: choice of field delimiter, options are: [,;:|/\]; [Output] section
     * 
    ...
    static std::vector<Coords> getUniqueLocations(NEADFile &file) {
    ...
    // ----------------- NEADIO -----------------
    NEADIO::NEADIO(const std::string &configfile)
    ...
    NEADIO::NEADIO(const Config &cfgreader)
    ...
    void NEADIO::parseInputSection() {
        ...
        if (in_meteo == "NEAD") { // keep it synchronized with IOHandler.cc for plugin mapping!!
            ...
            std::vector<std::string> all_files_and_paths = getFilesWithPaths(vecFilenames, inpath);
    ...
    void NEADIO::parseOutputSection() {
        ...
        if (out_meteo == "NEAD") { // keep it synchronized with IOHandler.cc for plugin mapping!!
            ...
            cfg.getValue("NEAD_APPEND", "Output", allow_append, IOUtils::nothrow);
            cfg.getValue("NEAD_OVERWRITE", "Output", allow_overwrite, IOUtils::nothrow);
            cfg.getValue("NEAD_SEPARATOR", "Output", out_delimiter,
            ...
            if (allow_overwrite && allow_append)
                throw InvalidFormatException("Cannot allow both NEAD_APPEND and NEAD_OVERWRITE", AT);
        }
    }

    // ------------------------------------- NEADIO read -----------------------------------------

    void NEADIO::readStationData(const Date & /*date*/, std::vector<StationData> &vecStation) {
        vecStation.clear();
        ...
        for (size_t ii = 0; ii < stations_files.size(); ii++) {
            StationData sd;
            NEADFile &current_file = stations_files[ii];
    ...
    void NEADIO::readMeteoData(const Date &start_date, const Date &end_date, std::vector<std::vector<MeteoData>> &vecvecMeteo) {
        vecvecMeteo.clear();
    ...
    // --------------------------- NEADIO read helper functions ---------------------------------------

    /**
     * @brief Reads the meta data from the NEAD file and populates the StationData object.
     *
     * This function reads the meta data from the NEAD file specified by current_file and populates
     * the StationData object specified by meta. It sets the meta data position, station ID, station name,
     * meta data slope, and extra information. If the location is not specified in the header of the NEAD file,
     * it sets the EPSG code for the position. The nodata value is used to handle missing data.
     *
     * @param current_file The NEAD file from which to read the meta data.
     * @param meta The StationData object to populate with the meta data.
     */
    void NEADIO::read_meta_data(const NEADFile &current_file, StationData &meta) {
    ...
    void NEADIO::setMetaDataPosition(const NEADFile &current_file, StationData &meta, const double &nodata_value) {
    ...
    void NEADIO::setMetaDataSlope(const NEADFile &current_file, StationData &meta, const double &nodata_value) {
     ...
     *
     * @param current_file The NEADFile object representing the file to read from.
     * @throws IOException if the file cannot be opened.
     */
    void NEADIO::readDataSequential(NEADFile &current_file) {
     ...
     * provided NEADFile, Date, and geoLocation information.
     *
     * @param current_file The NEADFile object containing the necessary data.
     ...
     */
    std::vector<MeteoData> NEADIO::createMeteoDataVector(NEADFile &current_file, std::vector<Date> &date_vec,
    ...
    void NEADIO::setMeteoDataLocation(MeteoData &tmp_md, geoLocation &loc, NEADFile &current_file, double nodata) {
    ...
    void NEADIO::setMeteoDataFields(MeteoData &tmp_md, NEADFile &current_file, Date &date, std::vector<size_t> &indexes, double nodata) {
        double offset = 0;
        double multiplier = 1;
    ...
    double NEADIO::getSnowpackSlope(const std::string &id) {
    ...
    /**
     * @brief Associate MeteoData parameter index with each NEAD field.
     * @details This function associates a parameter index for MeteoData objects with the
     * lineup of field types in NEAD FIELDS
     *
     ...
     */
    void NEADIO::identify_fields(const std::vector<std::string> &fields, std::vector<size_t> &indexes, MeteoData &md,
    ...
    void NEADIO::writeMeteoData(const std::vector<std::vector<MeteoData>> &vecvecMeteo, const std::string &) {
    ...
    // ----------------- NEADIO write helper functions -----------------
    /**
     * @brief Creates a filename for the NEAD file based on the given station data.
     *
     * Creates a filename based on the station id, and if none is provided, the internal index
     * of the station is used
     *
     * @param[out] outfile The NEADFile object to store the generated filename.
     ...
     */
    bool NEADIO::createFilename(NEADFile &outfile, const StationData &station, size_t ii) {
        outfile.METADATA.station_id = station.getStationID().empty() ? "Station" + IOUtils::toString(ii + 1) : station.getStationID();
        outfile.filename = outpath + "/" + outfile.METADATA.station_id + dflt_extension_NEAD;
     ...
     */
    void NEADIO::prepareOutfile(NEADFile &outfile, const std::vector<MeteoData> &vecMeteo, bool file_exists) {
        ...
        outfile.firstline = NEAD_firstline;
    ...
    void NEADIO::handleNewFile(NEADFile &outfile, const std::vector<MeteoData> &vecMeteo, bool file_exists) {
        if (file_exists && !allow_overwrite) {
            throw IOException("File " + outfile.filename + " already exists and NEAD_OVERWRITE is not allowed", AT);
        }
        createMetaDataSection(outfile, vecMeteo);
        createFieldsSection(outfile, vecMeteo);
    }

    void NEADIO::handleFileAppend(NEADFile &outfile, const std::vector<MeteoData> &vecMeteo) {
        if (outfile.location_in_header && outfile.station_location != toNEADLocation(vecMeteo[0].meta.position, outfile.METADATA.epsg)) {
            throw IOException("Inconsistent geographic coordinates between header and data in file \"" + outfile.filename + "\": " +
                                  outfile.station_location.toString() + " != " + toNEADLocation(vecMeteo[0].meta.position, outfile.METADATA.epsg).toString(),
                              AT);
        }
     ...
     *
     * @param outfile The NEADFile object representing the output file.
     * @param vecMeteo A vector of MeteoData objects containing the meteorological data.
     */
    void NEADIO::createMetaDataSection(NEADFile &outfile, const std::vector<MeteoData> &vecMeteo) {
    ...
    void NEADIO::createFieldsSection(NEADFile &outfile, const std::vector<MeteoData> &vecMeteo) {
    ...
    bool NEADIO::checkLocationConsistency(const std::vector<MeteoData> &vecMeteo) {
    ...
    std::string NEADIO::getGeometry(const geoLocation &loc) {
    ...
    void NEADIO::writeToFile(const NEADFile &outfile) {
