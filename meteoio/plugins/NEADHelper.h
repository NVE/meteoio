...
namespace mio {
    namespace NEAD {
        ...
        struct geoLocation {
            ...
            Coords toCoords(int epsg) const {
                Coords loc;
            ...
            geoLocation() = default;
            geoLocation(double in_x, double in_y, double in_z, double in_slope_angle=default_nodata, double in_slope_azi=default_nodata);
        ...
        struct MetaDataSection {
            ...
            void setEPSG(const int &in_epsg) {
                this->epsg = in_epsg;
                this->srid = "EPSG:" + std::to_string(in_epsg);
            }
        ...
        geoLocation toNEADLocation(Coords coords, int epsg);
        ...
        public:
            ...
            NEADFile(const std::string &infile, bool read_sequential);

            // Main methods
            void readFile(const std::string &infile, bool sequentially);
            ...
            // reading helpers
            bool processLine(const std::string& line, std::string& section, bool& sequentially);
            void processContent(const std::string& content, std::string& section);
            void processData(const std::string& content);

            // Format helpers
            bool isValidLocation(geoLocation location);
