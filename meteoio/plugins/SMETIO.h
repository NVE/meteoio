...
class SMETIO : public IOInterface {
	...
	private:
		...
		std::set<std::string> checkForUsedParameters(const std::vector<MeteoData>& vecMeteo, double& smet_timezone);
		...
		void generateHeaderInfo(const StationData& sd, const bool& i_outputIsAscii, const bool& isConsistent,
		                        const double& smet_timezone,
		                        const std::set<std::string>& paramInUse,
