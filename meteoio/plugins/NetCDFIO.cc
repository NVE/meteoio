...
{
	if (schema=="CF-1") return NC_DOUBLE;
	if (schema=="CNRM") return NC_DOUBLE;
	if (schema=="ECMWF") return NC_FLOAT;
...
{ //HACK: vars/dims should be identified based on standard_name, not name (cf1)
	...
	tmp.clear();
	tmp.push_back( ncpp::var_attr(ncpp::TIME, "time", "time", "", "s", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(ncpp::LATITUDE, "latitude", "latitude", "", "degree_north", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(ncpp::LONGITUDE, "longitude", "longitude", "", "degree_east", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(ncpp::STATION, "station", "timeseries_id", "", "", IOUtils::nodata, NC_CHAR) );
	tmp.push_back( ncpp::var_attr(ncpp::EASTING, "easting", "projection_x_coordinate", "", "m", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(ncpp::NORTHING, "northing", "projection_y_coordinate", "", "m", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::DEM, "surface_altitude", "surface_altitude", "height above mean sea level", "m", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::TA, "air_temperature", "air_temperature", "near surface air temperature", "K", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::RH, "relative_humidity", "relative_humidity", "", "", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::DW, "wind_from_direction", "wind_from_direction", "", "degree", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::VW, "wind_speed", "wind_speed", "", "m/s", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::P, "air_pressure", "air_pressure", "near surface air pressure", "Pa", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::ISWR, "downwelling_shortwave_flux_in_air", "downwelling_shortwave_flux_in_air", "", "W/m2", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::RSWR, "upwelling_shortwave_flux_in_air", "upwelling_shortwave_flux_in_air", "", "W/m2", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::ILWR, "downwelling_longwave_flux_in_air", "downwelling_longwave_flux_in_air", "", "W/m2", IOUtils::nodata, NC_DOUBLE) );
	results["CF-1"] = tmp;

	//CNRM schema
	tmp.clear();
	tmp.push_back( ncpp::var_attr(ncpp::TIME, "time", "time", "time", "", IOUtils::nodata, NC_DOUBLE) );
	...
	tmp.push_back( ncpp::var_attr(MeteoGrids::DEM, "ZS", "", "altitude", "m", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::SLOPE, "slope", "", "slope angle", "degrees from horizontal", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::AZI, "aspect", "", "slope aspect", "degrees from north", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::TA, "Tair", "", "Near Surface Air Temperature", "K", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::RH, "HUMREL", "", "Relative Humidity", "%", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::QI, "Qair", "", "", "", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::VW, "Wind", "", "Wind Speed", "m/s", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::DW, "Wind_DIR", "", "Wind Direction", "deg", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::PSUM_L, "Rainf", "", "Rainfall Rate", "kg/m2/s", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::PSUM_S, "Snowf", "", "", "", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::ISWR_DIR, "DIR_SWdown", "", "Surface Incident Direct Shortwave Radiation", "W/m2", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::ISWR_DIFF, "SCA_SWdown", "", "", "", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::P, "PSurf", "", "Surface Pressure", "Pa", IOUtils::nodata, NC_DOUBLE) );
	tmp.push_back( ncpp::var_attr(MeteoGrids::ILWR, "LWdown", "", "Surface Incident Longwave Radiation", "W/m2", IOUtils::nodata, NC_DOUBLE) );
	results["CNRM"] = tmp;
	...
	//WRF schema
	tmp.clear();
	tmp.push_back( ncpp::var_attr(ncpp::TIME, "Times", "Times", "Times", "", IOUtils::nodata, NC_CHAR) );
...
ncParameters::ncParameters(const std::string& filename, const Mode& mode, const Config& cfg, const std::string& schema, const double& tz_in, const bool& i_debug)
             : user_schemas( initUserSchemas(cfg) ), user_dimensions( initUserDimensions(cfg) ), vars(), unknown_vars(), vecTime(), vecX(), vecY(), dimensions_map(), file_and_path(filename), current_schema(schema), coord_sys(), coord_param(), TZ(tz_in), schema_dflt_type(initDfltType(schema)), wrf_hacks(schema=="WRF"), debug(i_debug), isLatLon(false)
{
...
void ncParameters::writeMeteo(const std::vector< std::vector<MeteoData> >& vecMeteo/*, const size_t& station_idx*/) //HACK
{
	...
	isLatLon = true; //HACK
	const bool multiple_stations = true;
	...
	if (multiple_stations) dimensions.push_back( ncpp::STATSTRLEN ); //this MUST be before STATION
	...
	for (size_t ii=0; ii<nc_variables.size(); ii++) {
		...
		if (vars[ param ].varid == -1) { //skip existing nc_variables
			...
			if (multiple_stations) vars[ param ].dimids.push_back( dimensions_map[ncpp::STATION].dimid );
	...
	for (size_t st=0; st<vecMeteo.size(); st++) {
		...
		for (size_t ii=0; ii<md.getNrOfParameters(); ii++) { //TODO check which parameters do not have any values!
			...
			if (vars.count(param)==0) { //ie unrecognized in loaded schema, adding it
				const ncpp::var_attr tmp_attr(param, md.getNameForParameter( ii ), IOUtils::nodata, NC_DOUBLE); //using NC_DOUBLE as default
...
std::vector<std::string> ncParameters::read_1Dvariable(const int& ncid, const size_t& param, const size_t& strMaxLen) const
{
	const std::map<size_t, ncpp::nc_variable>::const_iterator it = vars.find( param );
	if (it==vars.end() || it->second.varid==-1) throw InvalidArgumentException("Could not find parameter \""+ncpp::getParameterName(param)+"\" in file \""+file_and_path+"\"", AT);
	const size_t length = read_1DvariableLength(it->second);
	...
	for(size_t ii=0; ii<length; ii++) {
		std::string tmp(strMaxLen, '\0');
		for(size_t jj=0; jj<strMaxLen; jj++) tmp[jj] = data[ii*strMaxLen+jj];
...
size_t ncParameters::read_1DvariableLength(const ncpp::nc_variable& var) const
{
	...
	if ((var.attributes.type==NC_CHAR && ndims!=2) || (var.attributes.type!=NC_CHAR && ndims!=1)) throw InvalidArgumentException("Parameter \""+ncpp::getParameterName(var.attributes.param)+"\" in file \""+file_and_path+"\" is not a 1D variable", AT);
	
	const int dimid = var.dimids[0]; //in the case of vector of strings, the first dimension is the vector size
