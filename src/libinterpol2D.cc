...
//finds the linear regression y=ax+b for points (x,y)
//if ignore_index>=0, ignore given index (as a way to remove a point from the interpolation)
...
// 			printf("%g %g\n",X[i],Y[i]);
// 		}
		//THROW SLFException("Poor linear regression coefficient", AT);
	}
...
//linear lapse: coeffs must have been already computed
	if (coeffs.size()<1)
          THROW SLFException("Linear regression coefficients not initialized", AT);
...
void Interpol2D::calculate(CArray2D<double>& param_out) {
	...
	if(InputSize==0) {	//no data
		...
		if(flag_ok==0)
                  THROW SLFException("Wrong interpolation type for zero data source", AT);
	}
	...
	if(InputSize==1) {	//single data source
		...
		if(flag_ok==0)
                  THROW SLFException("Wrong interpolation type for single data source", AT);
	}
	...
	if(InputSize>1) {	//multiple data sources
		...
		if(flag_ok==0)
                  THROW SLFException("Wrong interpolation type for multiple data sources", AT);
	}
}
