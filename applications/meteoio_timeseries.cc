...
inline Timeseries parseCmdLine(int argc, char **argv)
{
	Config cfg;
	Date begin_date, end_date;
	bool showProgress;
	std::string cfgfile = "io.ini";
	...
	timeseries.setTimeoutSecs(timeout_secs);
	timeseries.setOutputBufferSize(outputBufferSize);
	timeseries.setShowProgress(showProgress);
	return timeseries;
}
